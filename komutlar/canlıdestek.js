...
exports.run = async (client, message, args) => {
  ...
  let mesaj = args.slice(0).join("682630064474685480");
  let chan = message.channel;
  let destekKanal = "682630064474685480";
  ...
  client.channels
    .get(destekKanal)
    .send(
      "📞 Destek çagrısı bağlanmak için `katıl` yazınız. İptal Etmek İçin `kapat` yazınız."
    );
  ...
  collector.on("end", (collected, reason) => {
    if (reason === "time")
      return message.reply("📞 Çagrı zaman aşımına uğradı.");
    if (reason === "aborted") {
    ...
    if (reason === "success") {
      ...
      client.channels
        .get(destekKanal)
        .send("📞 Destek çağrısını kapatmak için `kapat` yazınız.");
        ...
        function contact() {
          ...
          if (message.content.startsWith("kapat")) {
            ...
            if (message.channel.id === chan.id)
              client.channels
                .get(destekKanal)
                .send("📞 Çağrı karşı taraftan kapatıldı.");
            if (message.channel.id === destekKanal)
              chan.send("📞 Çağrı karşı taraftan kapatıldı.");
